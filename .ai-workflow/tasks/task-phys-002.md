...
## 目标（Goal）
在 `Engine.Physics` 中实现 `SceneDescription -> PhysicsWorld` 主路径、固定步进统计、只读 snapshot，以及 AABB overlap / ground query，让 M19 Physics foundation 具备真实数据流和可验证查询能力。
...
- M19.3 是 Physics foundation 的主实现卡；没有这张卡，`Engine.Physics` 只是模块骨架，SceneData physics schema 也无法进入真实 world / step / snapshot / query 主路径。
- 本卡承担的是 PhysicsWorld 的真实加载、fixed-step 统计、snapshot 与 query，不承担 App 主循环接入、Scene Transform 回写或可见物理效果。
- 直接影响本卡实现的上游背景包括：Physics 直接消费 `SceneDescription`；只有同时具备 `Transform + RigidBody + BoxCollider` 的对象进入 world；M19 忽略 rotation；M20 才做 App/Scene runtime behavior 与 Transform writeback。

## 决策继承（DecisionCarryOver）
- 从计划/里程碑继承的关键决策：
  - `PhysicsWorld.LoadFromDescription(SceneDescription)` 或等价入口必须存在。
  - 只有同时具备 `Transform`、`RigidBody`、`BoxCollider` 的对象进入 physics world。
  ...
  - `Step` 不修改输入 `SceneDescription`，也不回写 `Engine.Scene` Transform。
- 本卡执行时不得推翻的既定取舍：
...
- 若计划/里程碑已给出示例数据结构、字段草图、DTO/record/class 形状、关系图或字段命名约定：
  - `PLAN-M19-2026-05-03 > Data Flow` 已定稿 `Scene JSON -> SceneData normalize -> PhysicsWorld -> Step -> Snapshot/Query` 主路径，执行时不得绕回手工注册 bodies 主路径。
...
- 先实现 `LoadFromDescription(...)` 或等价 world 构建入口，消费真实 `SceneDescription` 中的 Transform / RigidBody / BoxCollider 描述。
...
- 最后补 `Engine.Physics.Tests`，至少覆盖：
  - realistic SceneData fixture -> PhysicsWorld 主路径
  - body count / ids / names / body types 稳定
  - AABB 计算
  - overlap / ground query
  - malformed input fail fast
  - no Transform writeback / no input SceneDescription mutation
...
- 若 world 构建入口名称需按仓库风格微调，允许等价命名，但必须仍然是从真实 `SceneDescription` 入手的主路径。
...
- Level: `L3`
- Why:
  - 这张卡是 M19 的真实核心实现，涉及真实数据流、world state、步进统计和查询语义，且边界选错就会直接滑入 M20。
...
- Test: `dotnet test tests/Engine.Physics.Tests/Engine.Physics.Tests.csproj --no-restore --nologo -v minimal` 通过，覆盖真实 fixture 加载、AABB、query、step 统计和 failure 语义
- Smoke: 至少一条真实 SceneData fixture 能走通 `SceneDescription -> PhysicsWorld -> Step -> Snapshot/Query` 主路径
